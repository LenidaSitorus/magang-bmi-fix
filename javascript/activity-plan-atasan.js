...
function searchTable() {
  ...
  const project = document.getElementById("searchProject").value.toLowerCase();
  const scope = document.getElementById("searchScope").value.toLowerCase();
  ...
  tableRows.forEach((row) => {
    ...
    const projectName = cells[1].innerText.toLowerCase();
    const scopeName = cells[4].innerText.toLowerCase();
    const scheduleStart = cells[7].innerText; // Kolom Jadwal Pelaksanaan Awal
    const scheduleEnd = cells[8].innerText; // Kolom Jadwal Pelaksanaan Akhir

    // Cek apakah baris memenuhi kriteria pencarian
    const matchesProject = project === "" || projectName.includes(project);
    const matchesScope = scope === "" || scopeName.includes(scope);
    const matchesStartDate = startDate === "" || scheduleStart >= startDate;
    const matchesEndDate = endDate === "" || scheduleEnd <= endDate;

    // Tampilkan atau sembunyikan baris
    if (matchesProject && matchesScope && matchesStartDate && matchesEndDate) {
